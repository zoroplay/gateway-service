...
import {
  SwaggerAddToSegmentRequest,
  SwaggerGrantBonusToSegment,
...
export class UsersController {
  ...
  @ApiOperation({
    ...
    description:
      'This endpoint is used to get lists  of all player segments for a clients',
  })
  @ApiOkResponse({ type: SwaggerCommonResponse })
  @ApiQuery({ name: 'clientId', description: 'SBE Client ID' })
  fetchPlayerSegments(@Query() clientId: FetchPlayerSegmentRequest) {
  ...
  @ApiOperation({
    ...
    description:
      'This endpoint is used to save or update player segment for a client',
  })
  ...
  async upload(
    ...
    const content = await workbook.xlsx.readFile(
      `${PATH_DOWNLOADED_FILE}/${file.filename}`,
    );
  ...
  @ApiOperation({
    ...
    description:
      'This endpoint is used to fetch all players for a particular segment',
  })
  ...
  @ApiOperation({
    ...
    description:
      'This endpoint is used to assing mass bonus to players under a segment',
  })
  @ApiParam({ name: 'segmentId', description: 'Segment ID' })
  @ApiBody({ type: SwaggerGrantBonusToSegment })
  @ApiOkResponse({ type: SwaggerCommonResponse })
  grantBonusToSegment(@Body() body: GrantBonusRequest, @Param() param) {
  ...
  @ApiOperation({
    ...
    description: 'This endpoint is used to update a user status',
  })
  @ApiQuery({ name: 'clientId', description: 'SBE Client ID' })
  ...
  @ApiBody({ type: SwaggerCommonResponse })
  updateUserStatus(
    @Param('id') id: string,
    @Body() body: { initialType: number; currentType: number },
  ) {
    const data = {
      userId: Number(id),
      ...body,
    ...
    return this.svc.changeUserTypeStatus(data);
  }
}
